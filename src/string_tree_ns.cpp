...
Node::Node(unsigned int in_edge_begin, unsigned int in_edge_len, unsigned int depth, unsigned int id,
           unsigned int fid)
    : in_edge_begin(in_edge_begin), in_edge_len(in_edge_len), depth(depth), id(id), fid(fid), parent(NULL) {}

void Node::info() const {
  std::cerr << " in_edge_begin=" << in_edge_begin << " in_edge_len=" << in_edge_len << " depth=" << depth
            << " id=" << id << " fid=" << fid << " children.size()=" << children.size();
  if (parent) {
...
Node* Tree::createNode(bool isFactor, unsigned int in_edge_begin, unsigned int in_edge_len,
                       unsigned int depth) {
  Node* node;
  if (isFactor) {
...
Node* Tree::getChild(const Node* node, unsigned int strbegin) const {
  ...
  if (candidate_child != NULL && canReachTo(candidate_child, strbegin))
...
bool Tree::canReachTo(const Node* node, unsigned int strbegin) const {
  ...
  assert(strbegin < this->getStrSize());
  if (node->depth == 1) {
    if (getInFirstChar(node) == (char)this->getCharAt(strbegin))
      return true;
    else
      return false;
  } else {
...
Node* Tree::insertFactorNode(Node* node, unsigned int strbegin, unsigned int len) {
  ...
  unsigned int length_to_last_node =
  ...
  Node* child;
  if (strpar_begin < this->getStrSize())
    child = this->getChildBeginWith(last_node, this->getCharAt(strpar_begin));
  else
    child = NULL;
...
unsigned int Tree::findLastNodeIdFrom(const Node* cur, unsigned long long strbegin, bool findFNode,
...
unsigned int Tree::LCPToNode(const Node* node, unsigned int strbegin, unsigned int len) const {
  ...
  if (node->depth == 1) {
    if (getInFirstChar(node) == str[strbegin])
      return 1;
    else
      return 0;
  }
