...
void Node::info() const {
  std::cerr << "label=[" << label << "]"
            << " depth=" << depth << " id=" << id << " fid=" << fid << " children.size()=" << children.size();
  if (parent) {
...
Tree::Tree(std::istream& is, std::ostream& os, unsigned int codeSize)
    : is(is), os(os), codeSize(codeSize), in_str_size(0), out_factor_size(0) {
  Init();
}
...
void Tree::compress() {
  ...
  while (is || !remain_string.empty()) {
    ...
    if (firstFactor == NULL) {
      ...
      if (UTIL::DEBUG_LEVEL > 1) {
        ...
        std::cerr << "[" << this->fnodes.size() << "]=[" << firstFactor->fid << "][" << cur->fid << "]=["
                  << left << "][" << right << "]" << std::endl;
      }
  ...
  if (firstFactor != NULL) {
    ...
    std::cerr << "[" << this->fnodes.size() << "]=[" << firstFactor->fid << "][" << this->root->fid << "]=["
              << left << "]["
              << "]" << std::endl;
    count_factors++;
    this->write(firstFactor->fid, this->root->fid);
  }
  std::cerr << "count_chars=" << count_in_chars << " count_factors=" << count_factors << std::endl;
}
...
void Tree::decompress() {
  ...
  while (is.read(reinterpret_cast<char*>(&first), sizeof(first)) &&
...
Node* Tree::getChild(const Node* node, std::string& label, unsigned int label_begin,
  ...
  if (candidate_child != NULL && canReachTo(candidate_child, label, label_begin, label_len))
...
Node* Tree::insertFactorNode(Node* node, std::string& label, unsigned int label_begin,
  ...
  unsigned int matched_len =
      last_node->depth - node->depth;  // the path length between a givne $node and $par.
...
unsigned int Tree::findLastNodeIdFrom(const Node* cur, bool findFNode, std::string& label,
...
unsigned int Tree::LCPToNode(const Node* node, std::string& label, unsigned int label_begin,
                             unsigned int label_len) const {
  ...
  if (node->depth == 1) {
    if (getInFirstChar(node) == label[label_begin])
      return 1;
    else
      return 0;
  }
...
inline void Tree::info() {
  std::cerr << "** TREE INFO:"
            << " num_nodes=" << this->num_nodes << " num_fnodes=" << this->num_fnodes << std::endl;
}
