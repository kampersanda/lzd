...
class STreeOcc : public STreeLimit {
 ...
 public:
  STreeOcc(std::istream& is, std::ostream& os, unsigned int codeSize)
      : STreeLimit(is, os, codeSize), vOcc(max_fnode, 0), vars(max_fnode, std::make_pair(0, 0)){};

  ~STreeOcc(){};

  inline unsigned int delIFfull() {
    if (UTIL::DEBUG_LEVEL > 1) {
      std::cerr << "DEBUG DEL num_fnodes=" << num_fnodes << " max_fnode=" << max_fnode << std::endl;
      if (this->isFull()) {
    ...
    while (this->isFull()) {
      for (unsigned int i = 1; i < this->max_fnode; i++) {
        if (UTIL::DEBUG_LEVEL > 2) {
          std::cerr << "i=" << i << " vOcc[i]=" << vOcc[i] << std::endl;
        }
  ...
  inline STree::Node* insertNewNode(STree::Node* node1, const STree::Node* node2,
    ...
    if (UTIL::DEBUG_LEVEL > 1) {
      ...
      std::cerr << "INSERT NEW NODE " << new_node->fid << " = (" << node1->fid << ")(" << node2->fid << ")"
                << std::endl;
    }
...
void STreeOccCompress(std::string& in_fname, std::string& out_fname, unsigned int codeSize) {
...
void STreeOccCompress(std::istream& is, std::ostream& os, unsigned int codeSize) {
  LZFF::FileInfo info(0, 0, codeSize, 0);
