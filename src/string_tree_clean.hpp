...
class STreeClean : public STreeLimit {
 public:
  STreeClean(std::istream& is, std::ostream& os, unsigned int codeSize) : STreeLimit(is, os, codeSize){};
  ...
  inline unsigned int delIFfull() {
    ...
    if (this->isFull()) {
      for (unsigned int i = 1; i < this->max_fnode; i++) {
        ...
        if (node != NULL) this->delNode(node);
      }
    }
    return 0;
  }
...
void STreeCleanCompress(std::string& in_fname, std::string& out_fname, unsigned int codeSize) {
...
void STreeCleanCompress(std::istream& is, std::ostream& os, unsigned int codeSize) {
  LZFF::FileInfo info(0, 0, codeSize, 0);
