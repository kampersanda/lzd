...
class Stat {
 public:
  ...
  Stat(unsigned int max_fnode = 0)
      : len_factors(0), len_lfactors(0), len_rfactors(0), len_deleted_factors(0), alive_time(0),
...
class Node {
 public:
  ...
  Node(unsigned int depth, unsigned int id, unsigned int fid, std::string& label)
      : depth(depth), id(id), fid(fid), label(label), parent(NULL){};

  Node(unsigned int depth, unsigned int id, unsigned int fid, std::string& label, unsigned int beg,
       unsigned int len)
      : depth(depth), id(id), fid(fid), label(label, beg, len), parent(NULL){};

  ~Node(){};

  inline unsigned int edgeLen() const {
    return label.size();
  };
  inline bool isFactor() const {
    return this->fid != UINT_MAX;
  }
...
class Tree {
 ...
 public:
  ...
  Node* lastFNode;
  Node* firstFactor;
  Tree(std::istream& is, std::ostream& os, unsigned int codeSize = 0);
  void Init();
  ...
  /**
   * creates a node whose depth is $depth, and also that in-edge label is
   * $str[$in_edge_begin..$in_edge_begin+$in_edge_len - 1]
   *
   * @param isFactor
   * @param in_edge_begin
   * @param in_edge_end
   ...
   * @return
   */
  inline Node* createNode(bool isFactor, unsigned int depth, std::string label, unsigned int label_begin,
                          unsigned int label_len) {
    Node* node;
    if (isFactor) {
      node = new Node(depth, this->getNewId(), this->getNewFId(), label, label_begin, label_len);
      registerFNode(node);
    } else {
      node = new Node(depth, this->getNewId(), UINT_MAX, label, label_begin, label_len);
    }
    registerNode(node);
    return node;
  }
  ...
  inline unsigned int getNumFNodes() {
    return num_fnodes;
  };
  ...
  inline Node* getFNode(unsigned int node_fid) const {
    return this->fnodes[node_fid];
  };
   ...
   * @return
   */
  Node* getChild(const Node* node, std::string& label, unsigned int label_begin,
                 unsigned int label_len) const;
   ...
   * @return
   */
  inline bool canReachTo(const Node* node, std::string& label, unsigned int label_begin,
                         unsigned int label_len) const {
    assert(node != NULL);
  ...
  inline Node* getParent(const Node* node) const {
    return node->parent;
  };
   ...
   * @return
   */
  inline Node* addChild(Node* parent, bool isFactor, std::string label, unsigned int label_begin,
                        unsigned int label_len) {
    assert(parent != NULL);
   ...
   * @param len
   */
  Node* insertFactorNode(Node* node, std::string& label, unsigned int beg, unsigned int len);
  ...
  inline Node* findLastNodeFrom(bool findFNode, const Node* node, std::string& label,
                                unsigned int label_begin, unsigned int label_len) const {
    return this->getNode(this->findLastNodeIdFrom(node, findFNode, label, label_begin, label_len));
  }
  inline Node* findLastNNodeFrom(Node* node, std::string& label, unsigned int label_begin,
                                 unsigned int label_len) {
  ...
  Node* findLastFNodeWithChar(const unsigned char new_char) {
    ...
    } else if (this->atNode(this->cur, this->len_matched) && this->cur->isFactor()) {
  ...
  /**
   * computes the longest common prefix between $str[$strbegin..$strbegin + $len - 1] and the label strings on
   * the paths from $node NOTE if @len = UINT_MAX, we recognize @len is the length of  str.
   *
   * @param node
   * @param strbegin
   * @param len
   ...
   * @return
   */
  unsigned int LCPToNode(const Node* node, std::string& label, unsigned int label_begin,
                         unsigned int label_len) const;
