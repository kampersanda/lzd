...
class STreeLimit : public STree::Tree {
 ...
 public:
  ...
  STreeLimit(std::istream& is, std::ostream& os, unsigned int codeSize)
      : Tree(is, os, codeSize), max_fnode(1 << this->codeSize), mask((1 << codeSize) - 1), out_bit(0),
        out_bit_size(8 * sizeof(this->out_bit)), codePairSize(2 * this->codeSize),
        NUM_BITS(sizeof(in_bit) * 8), remain_bitsize(NUM_BITS), in_bit(0), count_factors(0) {
    if (this->stat) {
  ...
  inline void write(unsigned int first, unsigned int second) {
    ...
    if (UTIL::DEBUG_LEVEL > 2) {
      std::cout << "[" << ++count_factors << "]=[" << first << "][" << second << "]" << std::endl;
    }
  ...
  void compress(LZFF::FileInfo& info) {
    ...
    while (is || !remain_string.empty()) {
      ...
      if (firstFactor == NULL) {
        ...
        const STree::Node* new_node = this->insertNewNode(firstFactor, cur, remain_string);
        ...
        if (UTIL::DEBUG_LEVEL > 1) {
          ...
          std::cerr << "[" << new_node->fid << "]=[" << firstFactor->fid << "][" << cur->fid << "]=[" << left
                    << "][" << right << "]" << std::endl;
        }
  ...
  inline std::pair<unsigned int, unsigned int> read() {
    ...
    if (remain_bitsize >= codePairSize) {
      ...
      const std::pair<unsigned int, unsigned int> res = std::make_pair(
          mask & (in_bit >> (remain_bitsize - codeSize)), mask & (in_bit >> (remain_bitsize - codePairSize)));
    ...
    remain_bitsize =
        NUM_BITS - (codePairSize - remain_bitsize);  // remain bit size after reading $codePairSize bits
  ...
  virtual STree::Node* insertNewNode(STree::Node* node1, const STree::Node* node2,
                                     std::string& remain_string) = 0;
  ...
  inline void delNode(STree::Node* node) {
    ...
    if (UTIL::DEBUG_LEVEL > 1) {
      ...
      std::cerr << "*** DELNODE *** [" << tmp << "] " << node->id << "(" << node->fid << ")" << std::endl;
  ...
  inline void info() const {
    std::cerr << "** TREE INFO:"
              << " num_nodes=" << this->num_nodes << " num_fnodes=" << this->num_fnodes
              << " num_total_nodes=" << this->num_total_nodes
              << " num_total_fnodes=" << this->num_total_fnodes << " max_fnode=" << this->max_fnode
              << std::endl;
  };
};
